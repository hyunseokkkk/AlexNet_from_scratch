...
//dimension에 맞게 shape normalize
/*vector<int> Tensor::normalize(const vector<int>& shape) const {
    vector<int> normalizedShape(shape); // 기본적으로 입력 shape 유지
    while (normalizedShape.size() < 4) {
        normalizedShape.insert(normalizedShape.begin(), 1); // 부족한 차원은 앞에서부터 1로 채움
    }
...
//1차원 인덱스를 다차원 인덱스로
vector<int> Tensor::IndexMuliDim(int flatIndex) const {
    if (flatIndex >= totalSize) {
        throw out_of_range("Flat index exceeds tensor size.");
    }

    vector<int> multiDimIndex(shape.size(), 0);
    for (size_t i = 0; i < shape.size(); ++i) {
        multiDimIndex[i] = flatIndex / strides[i];
        flatIndex %= strides[i];
    }
